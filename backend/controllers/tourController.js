...
export const getSingleTour = async (req, res) => {
    try {
        ...
        res.status(200).json({success: true, message: 'Tour found successfully', data: singleTour});
    }
    catch (error) {
...
export const getAllTour = async (req, res) => {
    ...
    try {
        const allTour = await Tour.find({}).populate('reviews').skip(page*9).limit(9);
        res.status(200).json({success: true, count: allTour.length, message: 'Tours found successfully', data: allTour});
    }
    catch (error) {
        res.status(404).json({success: false, message: 'not found'});
    }
...
export const getFeaturedTour = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({success: true, message: 'Featured Tours found successfully', data: featuredTour});
    }
    catch (error) {
        res.status(404).json({success: false, message: 'not found'});
    }
...
export const getTourBySearch = async (req, res) => {
    ...
    try {
        const tours = await Tour.find({name: {$regex: search, $options: 'i'}}).populate('reviews');
...
export const getTourCount = async (req, res) => {
    try {
        ...
        res.status(200).json({success: true, message: 'Tours count found successfully', data: count});
    }
    catch (error) {
        res.status(404).json({success: false, message: 'not found'});
    }
